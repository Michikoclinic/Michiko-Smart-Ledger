...
function renderDetailContent(line){let content=esc(formatNumbersInText(line)).replace(/support/gi,match=>`<strong class="support-text">${match}</strong>`).replaceAll("หักจากมัดจำ",'<strong class="deposit-deduction-text">หักจากมัดจำ</strong>').replace(/ค้างชำระ(?:\s*[:=]?\s*[\d,]+(?:\.\d+)?(?:\s*บาท)?)?/g,match=>`<strong class="outstanding-text">${match}</strong>`);content=content.replace(/หักจาก(?:เครดิต|วงเงินสลายฟิลเลอร์).*$/u,match=>`<strong class="credit-deduction-text">${match}</strong>`);return content}
