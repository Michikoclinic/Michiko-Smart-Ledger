...
cashReceiptForm.addEventListener("submit",event=>{event.preventDefault();const form=new FormData(cashReceiptForm);const amount=parseMoney(form.get("cash"));if(amount<=0){alert("กรุณากรอกจำนวนเงินสดให้ถูกต้อง");return}pendingCashReceipt={version:2,locked:true,branch:cashReceiptBranchName(),date:localDateKey(),hn:normalizedHn(form.get("hn")),patient:String(form.get("patient")||"").trim(),cash:amount,signature:null,signedAt:null,createdAt:new Date().toISOString()};document.querySelector("#reviewCashContext").textContent=`${rangeDateLabel(pendingCashReceipt.date)} · ${paymentBranchLabel(pendingCashReceipt.branch)}`;document.querySelector("#reviewCashHn").textContent=pendingCashReceipt.hn;document.querySelector("#reviewCashPatient").textContent=pendingCashReceipt.patient;document.querySelector("#reviewCashAmount").textContent=`${money(pendingCashReceipt.cash)} บาท`;confirmCashReceipt.checked=false;saveCashReceiptPermanent.disabled=true;cashReceiptModal.hidden=true;cashReceiptReviewModal.hidden=false});
...
cashReceiptRows.addEventListener("click",event=>{const button=event.target.closest("[data-cash-sign-key]");if(button)openCashSignatureModal(button.dataset.cashSignKey)});
