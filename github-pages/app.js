...
function formatNumbersInText(text){return String(text).replace(/\d[\d,]*/g,(token,offset,source)=>{const previous=source[offset-1]||"";const next=source[offset+token.length]||"";if(/[A-Za-z]/.test(previous)||/[A-Za-z]/.test(next))return token;const digits=token.replace(/,/g,"");return digits.length>=4?Number(digits).toLocaleString("en-US"):token})}
