...
function renderDetailContent(line){const formatted=formatNumbersInText(line);if(/หักจาก(?:เครดิต|วงเงินสลายฟิลเลอร์)/u.test(formatted))return `<strong class="credit-deduction-text">${esc(formatted)}</strong>`;let content=esc(formatted).replace(/support/gi,match=>`<strong class="support-text">${match}</strong>`).replaceAll("หักจากมัดจำ",'<strong class="deposit-deduction-text">หักจากมัดจำ</strong>').replace(/ค้างชำระ(?:\s*[:=]?\s*[\d,]+(?:\.\d+)?(?:\s*บาท)?)?/g,match=>`<strong class="outstanding-text">${match}</strong>`);return content}
