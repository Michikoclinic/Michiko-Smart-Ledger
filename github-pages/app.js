...
function openCashReceiptEditor(storageKey){try{const record=JSON.parse(localStorage.getItem(storageKey)||"null");if(!record||record.locked!==true)return;resetCashReceiptForm();editingCashReceiptKey=storageKey;document.querySelector("#cashReceiptFormTitle").textContent="แก้ไขข้อมูลรับเงินสด";document.querySelector("#cashReceiptFormContext").textContent=`${rangeDateLabel(record.date)} · ${paymentBranchLabel(record.branch)}`;document.querySelector("#cashReceiptFormHelper").textContent=record.signature?"แก้ไขได้เฉพาะ HN ชื่อ และยอดเงินสด ลายเซ็นและเวลาเดิมจะไม่เปลี่ยนแปลง":"หลังบันทึกการแก้ไขยังสามารถกดช่องลายเซ็นเพื่อเซ็นรับได้";cashReceiptForm.elements.hn.value=record.hn||"";cashReceiptForm.elements.patient.value=record.patient||"";cashReceiptForm.elements.cash.value=formatMoneyInput(record.cash)||"0";cashReceiptModal.hidden=false;setTimeout(()=>cashReceiptForm.elements.hn.focus(),0)}catch{alert("เปิดข้อมูลสำหรับแก้ไขไม่สำเร็จ")}}
