...
function additionPreviews(text){const results=[];String(text).replace(/((?:\d[\d,]*\s*\+\s*)+\d[\d,]*)/g,expression=>{const values=expression.match(/\d[\d,]*/g)||[];if(values.length>1){const sum=values.reduce((total,value)=>total+parseMoney(value),0);results.push(`${expression.replace(/\s*\+\s*/g," + ")} = ${Number(sum).toLocaleString("en-US")}`)}return expression});return results}
