...
function renderDetailContent(line){return esc(formatNumbersInText(line)).replaceAll("หักจากเครดิต",'<strong class="credit-deduction-text">หักจากเครดิต</strong>').replace(/support/gi,match=>`<strong class="support-text">${match}</strong>`).replaceAll("หักจากมัดจำ",'<strong class="deposit-deduction-text">หักจากมัดจำ</strong>').replace(/ค้างชำระ(?:\s*[:=]?\s*[\d,]+(?:\.\d+)?(?:\s*บาท)?)?/g,match=>`<strong class="outstanding-text">${match}</strong>`)}
