...
detailInput.addEventListener("input",()=>{const cursor=detailInput.selectionStart??detailInput.value.length;phraseQuery=detailInput.value.slice(detailInput.value.lastIndexOf("\n",cursor-1)+1,cursor).trim();showAllPhrases=false;renderPhrases()});
function resetEditor(){entryForm.reset();editingIndex=null;phraseQuery="";showAllPhrases=false;document.querySelector("#saveEntryButton").textContent="บันทึกรายการ";entryForm.querySelectorAll(".payment-option").forEach(option=>option.classList.remove("selected"));entryForm.querySelectorAll(".payment-amount").forEach(input=>input.disabled=true);renderPhrases()}
